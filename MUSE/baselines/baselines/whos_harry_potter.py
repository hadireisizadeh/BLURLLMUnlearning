...
class WHPModelForCausalLM(PreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, return_dict=True, **kwargs):
        ...
        # Debug: Check input IDs and embedding size for baseline
        print(f"Baseline: Input IDs max: {input_ids.max().item()}, Embedding size: {self.baseline.get_input_embeddings().weight.size(0)}")
    
    # Debug: Check input IDs and embedding size for reinforced
        print(f"Reinforced: Input IDs max: {input_ids.max().item()}, Embedding size: {self.reinforced.get_input_embeddings().weight.size(0)}")
