...
#from utils import load_tokenizer, write_csv, read_json, write_json
...
#def load_model(model_dir: str, cache_dir: str = '.cache'):
 #   """Loads a model from the specified directory."""
  #  if not os.path.isdir(model_dir):
   #     raise ValueError(f"Model directory {model_dir} does not exist or is not a directory.")
...
def eval_model(
    model: AutoModelForCausalLM,#LlamaForCausalLM,
    tokenizer: AutoTokenizer = LLAMA_DIR,#LlamaTokenizer = LLAMA_DIR,
...
def load_then_eval_models(
    ...
    for model_dir, name in zip(model_dirs, names):
        model = load_model(model_dir,cache_dir='.cache')
