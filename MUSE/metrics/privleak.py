...
def sweep(ppl, y):
    ...
    #if np.isnan(ppl).any():
     #   print("Warning: NaN values detected in ppl. Replacing with np.inf.")
     #   ppl = np.nan_to_num(ppl, nan=np.inf)

 #   if np.isnan(y).any():
 #       raise ValueError("y contains NaN values, which is unexpected for labels.")
